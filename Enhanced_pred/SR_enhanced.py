...
def Super_resolution_gray(img):
    ...
    D, knn = K_nearest_Gaussian_SSD(patches_db, input_patches_p)
...
def Super_resolution_color(bgr_img):
    ...
    YCrCb_1 = cv2.resize(yCrCb_img,None,fx=2,fy=2)
    g_img_1 = g_img
    img90 = cv2.rotate(g_img_1, cv2.cv2.ROTATE_90_CLOCKWISE)
    img180 = cv2.rotate(g_img_1, cv2.ROTATE_180)
    img270 = cv2.rotate(g_img_1, cv2.cv2.ROTATE_90_COUNTERCLOCKWISE)
    h_flip = cv2.flip(g_img_1, 0)
    v_flip = cv2.flip(g_img_1, 1)
    b_flip = cv2.flip(g_img_1, -1)
    print('1')
    g_img_1 = Super_resolution_gray(g_img_1)
    print('2')
    img90 = Super_resolution_gray(img90)
    print('3')
    img180 = Super_resolution_gray(img180)
    print('4')
    h_flip = Super_resolution_gray(h_flip)
    print('5')
    v_flip = Super_resolution_gray(v_flip)
    print('6')
    img270 = Super_resolution_gray(img270)
    print('7')
    b_flip = Super_resolution_gray(b_flip)
    print('8')

    img90 = cv2.rotate(img90, cv2.cv2.ROTATE_90_COUNTERCLOCKWISE)
    img180 = cv2.rotate(img180, cv2.ROTATE_180)
    img270 = cv2.rotate(img270, cv2.cv2.ROTATE_90_CLOCKWISE)
    h_flip = cv2.flip(h_flip, 0)
    v_flip = cv2.flip(v_flip, 1)
    b_flip = cv2.flip(b_flip, -1)

    g_img_1 = (g_img_1 + img90+ img180+img270+h_flip+v_flip+b_flip)/7
    plt.imshow(g_img_1,cmap='gray')

    YCrCb_1[:,:,0]=(g_img_1*255).astype(np.uint8)
    rgb=cv2.cvtColor(YCrCb_1, cv2.COLOR_YCrCb2BGR)
